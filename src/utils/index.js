...
var condenseData = ({allRegionalVotes, region}) => {
  ...
  if (region.toLowerCase() === "scotland") {
      ...
      .reduce((acc, cur) => {
        ...
        return {
          ...
          [cur.name]: {
            ...
            "constituencySeats": acc[cur.name].constituencySeats + cur.constituencySeats,
            "listVoteCount": acc[cur.name].listVoteCount + cur.listVoteCount,
            "regionalSeats": acc[cur.name].regionalSeats + cur.regionalSeats,
            "totalSeats": acc[cur.name].constituencySeats + cur.constituencySeats + acc[cur.name].regionalSeats + cur.regionalSeats
          }
        }
      }, {
        "snp":
    ...
    partyInfo = regionSelectionData.parties
      .map(party => {
        return {
          name: party.name, 
          constituencySeats: party.constituencySeats,
    ...
    regionMetaData = {
      ...
      electorate: format({suffix: ' eligible voters'})(regionSelectionData.region.votes.electorate.toFixed(0)),
      turnout: (regionSelectionData.region.votes.total / regionSelectionData.region.votes.electorate * 100).toFixed(1) + "%",
    }
  }

  var totals = partyInfo.reduce((a, c) => {
    var _listVoteCount =  parseFloat(a.listVoteCount.replace(/,/g, '')) + parseFloat(c.listVoteCount.toString().replace(/,/g, ''));
...
var SplitTheVote = ({snpRatio = 1, region = 'scotland'}) => {
  ...
  const allRegionalVotes = Object
    .entries(modifiedRegions)
    .reduce((acc, [regionName, regionData]) => {
  
      return {
        ...acc,
        [regionName]: new Vote({
          name: regionName,
          parties: Parties.map(conf => { return new Party(conf) }),
          region: regionData
        })
      }
    }, {});
